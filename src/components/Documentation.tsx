...
import { Book, Code, FileText, BookOpen, TrendingUp, CheckCircle, Heart } from 'lucide-react';
...
const documentationItems = [
  ...
  {
    title: 'Benefits Integration Guide',
    description: 'Learn how to integrate with our comprehensive Benefits APIs.',
    icon: Heart,
    link: '#benefits-guide',
    iconColor: '#c8102e' // WEX red
  },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <TransitionContainer animation="fade-in-left">
            ...
            <p className="text-lg text-foreground/70 mb-8">
              Our documentation is designed to help you integrate with WEX APIs quickly and efficiently. From fleet management to payment processing and benefits administration, we've got you covered.
            </p>
            ...
            <AnimatedCard className="p-0 overflow-hidden">
              <div className="p-6 bg-wex-red text-white">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xl font-bold">Benefits API Authentication</h3>
                  <div className="bg-white/20 text-white text-xs rounded-full px-3 py-1">
                    Security
                  </div>
                </div>
                <p className="text-white/80 mb-4">
                  Learn how to authenticate your Benefits API requests using OAuth 2.0
                </p>
              </div>
              <div className="p-6">
                <div className="bg-foreground/5 dark:bg-foreground/10 rounded-lg p-4 font-mono text-sm overflow-auto mb-6">
                  <pre ref={codeRef} className="language-javascript">
                    <code>{`// Request an access token for Benefits APIs
const response = await fetch(
  'https://auth.wex.com/benefits/oauth/token',
  {
    method: 'POST',
    headers: {
    ...
    body: new URLSearchParams({
      ...
      'scope': 'benefits:read benefits:write healthcare:claims'
    })
  }
);
...
const { access_token } = await response.json();`}</code>
                ...
                <div className="mt-6 pt-4 border-t border-border">
                  <a href="#auth-guide" className="text-wex-red font-medium hover:underline transition-all duration-200">
                    View complete Benefits API authentication guide →
                  </a>
                </div>
              </div>
            </AnimatedCard>
